...
def findAllSolutions (requests)
	...
	requests.each do |request|
		graph.add_edge(request.orig_section_id, request.target_section_id)
		courseHash[[request.orig_section_id, request.target_section_id]] = request.course_id
		studentHash[[request.orig_section_id, request.target_section_id]] = request.student_id
		requestHash[[request.orig_section_id, request.target_section_id]] = request.id
	end
	...
	while i < cycles.length-1 do
		...
		while j < cycles.length do
			if (validCycle(cycles[i], cycles[j], studentHash, courseHash) == true)
	...
	while do_break == false
		do_break = true
		next_level = []
		last_level = all_solutions[all_solutions.length-1]
...
def findBestSolution (requests)
	...
	all_cycles.each do |cycles_list|
		...
		length_solutions.push(to_append)
	end
	...
	return fulfilled_values[all_times.index(all_times.max)]
